			...
			if(i < iplen - 1) {
				...
				for(int j = iplen - 1; j >= i; j--) {
					...
					if(val == -1) {
						...
						shift += 4;
						num[cyc] += val;
System.out.println("val of num" + cyc + ": " + num[cyc]);
					}
				}
			} else {
				while(cyc < 8) {
...
//System.out.println("between: high: " + Long.toHexString(ip.getHigh()) + " low: " + Long.toHexString(ip.getLow()));
		switch(mode) {
		case ZERO:
			break;
		case RANDOM:
...
//System.out.println("high: " + Long.toHexString(ip.getHigh()) + " low: " + Long.toHexString(ip.getLow()));
			break;
		default:
...
//System.out.println("first " + hash.get(num));
		if(ip == null) {
			ip = new IPv6_Int();
		...
		appendIP(ip, msg);
//System.out.println("test " + Long.toHexString(ip.getHigh()) + ", " + Long.toHexString(ip.getLow()));
	}
	
	@Override
	public void anon(CurrMsg msg) {
		if(syntax(msg)) {
//System.out.println("ipv6 prop: " + mode + " " + bits);
...
System.out.println("mode: " + mode + " bits: " + bits + " cons: " + cons);
	}
